...
# 以下是和具体任务相关的内容
set targetDirectory /sw/obstacleAvoidance/calculateKittiRawDisparity

# create directories on sw machine.
$sshCommand zb@$serverName mkdir -p  $targetDirectory

# copy all python source files to the sw machine.
# $scpCommand  -r *   zb@$serverName:$targetDirectory
# During developing, only the main python program and the main script are needed to be copied.
$scpCommand  *.py   zb@$serverName:$targetDirectory
